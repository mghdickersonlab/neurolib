...
def compute_dvr_image(pet_file, gtmpvc_dir, frame_start_times, time_window_start_end,
    ...
    spm_path = get_spm_path(spm_path)
    return matlab.run(
        f"addpath {spm_path}; addpath {MATLAB_SRC_DIR}; compute_dvr_image('{pet_file}', '{gtmpvc_dir}', '{frame_start_times}', '{time_window_start_end}', '{ref_roi}', '{weighted_merge}', '{output_image}')")
